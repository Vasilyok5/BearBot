...
(async () => {
    if (process.version.slice(1).split('.')[0] < 16) {
        ...
        logger.error("BearBot requires Node 16 or higher. Please update your version of Node.");
        process.exit(0);
    }
...
async function checkUpdate() {
    ...
    try {
        const res = await axios.get('https://raw.githubusercontent.com/Vasilyok5/BearBot/main/package.json');
...
async function main() {
    ...
    child.on("close", async (code) => {
        ...
        if (code !== 0 && restartCount < 5) {
            ...
            logger.warn("Restarting BearBot Workers Handler...");
            await new Promise(resolve => setTimeout(resolve, 2000));
            main();
        } else {
            console.log();
            logger.error("BearBot has been closed.");
            process.exit(0);
        }
    });
};
